...
//$to_date = (isset($_GET['chart_daily_toDate'])) ? $_GET['chart_daily_toDate'] : date( 'Y-m-d' );
// Check this out: htmlspecialchars()
...
"SELECT CONCAT( ?, ' ', b.time ) AS date,
 ...
 FROM {$database->table_prefix}time_index b
 LEFT JOIN {$database->table_prefix}temperatures a
...
//$log->logInfo( "draw_daily.php: Executing sqlTwo ($sqlTwo) for values $start_date, $start_date, $start_date, $start_date, $end_date, $end_date, $start_date, $end_date, $uuid" );
  ...
  $queryThree = $user->runQuery( $sqlThree );
...
if( $show_setpoint == 1 ){
  ...
  $queryFour = $user->runQuery( $sqlFour );
