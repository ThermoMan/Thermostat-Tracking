...
$sql = "SELECT date, heat_runtime, cool_runtime,
       (SELECT MAX(outdoor_temp)
        ...
        WHERE {$dbConfig['table_prefix']}run_times.date = DATE( {$dbConfig['table_prefix']}temperatures.date) AND tstat_uuid = ? ) AS outdoor_max,
       (SELECT MIN(outdoor_temp)
        FROM {$dbConfig['table_prefix']}temperatures
        WHERE {$dbConfig['table_prefix']}run_times.date = DATE( {$dbConfig['table_prefix']}temperatures.date) AND tstat_uuid = ? ) AS outdoor_min
       FROM {$dbConfig['table_prefix']}run_times WHERE tstat_uuid = ?
       ORDER by date ASC";
$query = $pdo->prepare( $sql );
$query->execute( array( $uuid, $uuid, $uuid ) );
...
$myPicture->autoOutput( 'images/runtime_chart.png' );
?>