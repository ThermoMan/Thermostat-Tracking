...
class USER{
  ...
  public function __construct( $uname = null, $session = null ){
    ...
    if( $this->uname != null && $this->session != null ){
      ...
      $stmt = $this->conn->prepare( "SELECT stat.id, stat.tstat_uuid, stat.model, stat.fw_version, stat.wlan_fw_version, stat.ip, stat.name, stat.description, stat.user_id, stat.zip_code, stat.timezone FROM {$this->database->table_prefix}thermostats AS stat, {$this->database->table_prefix}users AS user WHERE user.user_id = stat.user_id AND user.user_name = :uname" );
      ...
      $stmt = $this->conn->prepare( "SELECT mtu.user_id, mtu.mtu_id, mtu.ip, mtu.name, mtu.description FROM elec__meters AS mtu, {$this->database->table_prefix}users AS user WHERE user.user_id = mtu.user_id AND user.user_name = :uname" );
      $stmt->bindParam( ':uname', $this->uname );
      $stmt->execute();
  ...
  // Why is this called runSQL?  It doesn't run the SQL....
...
$key = '12345';
      ...
      $message = "Please click this <a href='https://freitag.theinscrutable.us/thermo2/confirm.php?key=$key'>link</a> to confirm your registation.";
