...
<head>
  ...
  <link href='lib/tabs/tabs.css' rel='stylesheet' type='text/css' />  <!-- Add Blixt tab library -->
  <!-- Follow up on this for a differnt way to do tabs http://www.w3.org/Style/Examples/007/target -->
  ...
  <script type='text/javascript'>
    ...
    function showRefreshTime()
    {
      ...
      //document.getElementById( 'daily_update' ).innerHTML = h + ':' + m + ':' + s;
      document.getElementById( 'daily_update' ).innerHTML = h + ':' + m;
    }
...
<div class='content' id='daily'>
  ...
  <!-- Need to change the max value to a date computed by Javascript so it stays current -->
    <input type='date' id='show_date' value='<?php echo $show_date; ?>' max='<?php echo $show_date; ?>' onInput='update_daily_chart();' step='1'/>
    <button type='button' name='show_date' onclick='javascript: show_date.stepUp(); update_daily_chart();'>--&gt;</button>
    <input type='checkbox' id='show_heat_cycles' name='show_heat_cycles' onChange='update_daily_chart();'/>Show Heat Cycles
    <input type='checkbox' id='show_cool_cycles' name='show_cool_cycles' onChange='update_daily_chart();'/>Show Cool Cycles
    <input type='checkbox' id='show_fan_cycles'  name='show_fan_cycles'  onChange='update_daily_chart();'/>Show Fan Cycles
    <input type='checkbox' id='auto_refresh'     name='auto_refresh'     onChange='timedRefresh();'/>Auto refresh (every 20 minutes)
    <span id='daily_update' style='visibility: hidden;'>00:00</span>
    <button class="refresh" type='button' onClick='update_daily_chart();'>Refresh</button>
  </div>
  <br>
  ...
  <br>CSS positioning of 'Refresh' button is not correct in Firefox.
</div>
