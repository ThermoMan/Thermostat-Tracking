...
//$pdo->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION, PDO::ERRMODE_WARNING );
  ...
  $sql = "SELECT stat.id, stat.tstat_uuid, stat.model, stat.fw_version, stat.wlan_fw_version, stat.ip, stat.name, stat.description, stat.user_id, stat.zip_code, stat.timezone FROM {$database->table_prefix}thermostats AS stat";
...
try{
  ...
  $sql = "UPDATE {$database->table_prefix}thermostats SET tstat_uuid = ?, description = ?, model = ?, fw_version = ?, wlan_fw_version = ? WHERE id = ?";
  $updateStatInfo = $pdo->prepare( $sql );

  $sql = "UPDATE {$database->table_prefix}hvac_status SET date = ?, start_date_heat = ?, start_date_cool = ?, start_date_fan = ?, heat_status = ?, cool_status = ?, fan_status = ? WHERE tstat_uuid = ?";
  $updateStatStatus = $pdo->prepare( $sql );

  $sql = "INSERT INTO {$database->table_prefix}hvac_cycles( tstat_uuid, system, start_time, end_time ) VALUES( ?, ?, ?, ? )";
  $cycleInsert = $pdo->prepare( $sql );

  // Query to retrieve prior setpoint.  Might find nothing if this is the first time.
  $sql = "SELECT set_point FROM {$database->table_prefix}setpoints WHERE id=? ORDER BY switch_time DESC LIMIT 1";
  $getPriorSetPoint = $pdo->prepare( $sql );

  $sql = "INSERT INTO {$database->table_prefix}setpoints( id, set_point, switch_time ) VALUES( ?, ?, ? )";
...
foreach( $allThermostats as $thermostatRec ){
  ...
  if( flock($lock, LOCK_EX) ){
    try{
      ...
      //$util::logDebug( "status: Connecting to Thermostat ID = ({$thermostatRec['id']})  uuid  = ({$thermostatRec['tstat_uuid']}) ip = ({$thermostatRec['ip']}) name = ({$thermostatRec['name']})" );
...
/* 2014-04-28 commented out.  Maybe only check model once every 30 minutes in the other routine?
      ...
      { // An error here is non-fatal, simply decline to use this info
        $util::logError( 'status: Thermostat failed to respond with model number.' );
      }
*/

/* 2014-04-28 commented out.  Maybe only check name once every 30 minutes in the other routine?
      $stat->getSysName();
      if( $stat->connectOK != 0 )
      { // An error here is non-fatal, simply decline to use this info
        $util::logError( 'status: Thermostat failed to respond with system info.' );
      }
*/
// Declining to update the thermostat info for now because a 0 or null in the uuid will screw up record collection the next time for this stat
      //$util::logDebug( "status: Updating thermostat record {$thermostatRec['id']}: UUID $stat->uuid DESC $stat->sysName MDL $stat->model FW $stat->fw_version WLANFW $stat->wlan_fw_version" );
...
// Also proper fix includes not trying to insert NULLs and catching SQL errors when something happens like that (and log it)
        ...
        //$util::logDebug( "status:  uuid = ($stat->uuid) GOT PRIOR STATE H $priorHeatStatus C $priorCoolStatus F $priorFanStatus SDH $priorStartDateHeat SDC $priorStartDateCool SDC $priorStartDateFan" );
        ...
        if( $priorHeatStatus && !$heatStatus ){
          //$util::logDebug( "status: uuid = ($stat->uuid) Finished Heat Cycle - Adding Hvac Cycle Record for $stat->uuid 1 $priorStartDateHeat $now" );
          $cycleInsert->execute( array( $stat->uuid, 1, $priorStartDateHeat, $now ) );
          $newStartDateHeat = null;
        }
        if( $priorCoolStatus && !$coolStatus ){
          //$util::logDebug( "status: $stat->uuid Finished Cool Cycle - Adding Hvac Cycle Record for $stat->uuid 2 $priorStartDateCool $now" );
          $cycleInsert->execute( array( $stat->uuid, 2, $priorStartDateCool, $now ) );
          $newStartDateCool = null;
        }
        if( $priorFanStatus && !$fanStatus ){
          //$util::logDebug( "status: $stat->uuid Finished Fan Cycle - Adding Hvac Cycle Record for $stat->uuid 3 $priorStartDateFan $now" );
    ...
    $util::logError( "status: Couldn't get file lock for thermostat {$thermostatRec['id']}" );
  }
  fclose( $lock );
}
$util::logDebug( 'status: End.  Execution time was ' . (microtime(true) - $start_time) . ' seconds.' );
